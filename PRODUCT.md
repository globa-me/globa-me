# GitHub Profile Product Context

## Purpose

This repository powers the public GitHub profile for `github.com/globa-me`. It is a compact personal landing page and an additional authoritative source about Gennadiy Zakharov on the open web.

## Positioning

Gennadiy is a creative digital marketing specialist first, with product-building and software as extensions of his marketing practice. The profile should not frame him as a conventional software engineer or list technology for its own sake.

Core idea: **digital marketing specialist working with AI and digital products.** The profile should state the work and evidence directly, without self-evaluative promises.

## Audiences

- Potential consulting and product-marketing clients.
- AI and digital-product teams.
- Creative, marketing, and education collaborators.
- People arriving from Gennadiy's social content about AI.

## Proof points

- Digital marketing practice since 2016.
- 300+ delivered projects.
- 56.3K followers across TikTok, Instagram, YouTube, and Facebook; 1.27M combined reported reach. Platform breakdown is maintained in `README.md`.
- Work across Europe and Asia, in Russian and English.
- Selected experience: Goethe-Institut, GPTunneL AI, Philip Morris International, Chocofood, AskDou.ai, Storista AI.

## Featured public products

- GZWhisper
- Link2Download
- SubCopy
- Screendance

## Source of truth

Use `https://zakharov.asia/` and its English version for biography, metrics, experience, portfolio, and current links. Never invent performance claims or imply that private/in-development products are publicly available.
